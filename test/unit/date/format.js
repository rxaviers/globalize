define([
	...
	ruCaGregorian, likelySubtags, timeData, weekData, metaZones, ianaTimezoneData ) {
...
QUnit.test( "should format timezone (z)", function( assert ) {
	...
	// FIXME need date that isDST === false.
	date = new Date( 2017, 0, 1 );
	...
	// FIXME need date that isDST === true.
...
QUnit.test( "should format timezone (v)", function( assert ) {
	...
	assert.dateFormatWithTimezone( date, "v", "Foo/Bar", cldr, [{
		type: "zone",
		value: "GMT"
	}]);
	assert.dateFormatWithTimezone( date, "vvvv", "Foo/Bar", cldr, [{
		type: "zone",
		value: "GMT"
	}]);
});
