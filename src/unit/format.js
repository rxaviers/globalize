...
return function( value, unit, options, cldr, globalize ) {
	...
	if ( (/-per-|\//).test( unit ) ) {
		...
		// divisor." UTS#35
		dividend = globalize.formatMessage( globalize.formatPlural( value, ret[ 0 ] ), value );
		divisor = globalize.formatMessage( globalize.formatPlural( 1, ret[ 1 ] ), "" ).trim();
		return globalize.formatMessage( cldr.main( [ "units", form, "per/compoundUnitPattern" ] ), [ dividend, divisor ] );
	}

	return globalize.formatMessage( globalize.formatPlural( value, ret ), value );
};
